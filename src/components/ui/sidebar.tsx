...
>(({ as: Comp = "button", asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, children, ...props }, ref) => {
  ...
  const button = (
    <Comp ref={ref} data-sidebar="menu-button" data-size={size} data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props}>
      {React.Children.map(children, (child) =>
        React.isValidElement(child) && child.type === "span" ? <span className="truncate">{child.props.children}</span> : child
      )}
    </Comp>
  );
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Use a consistent width for SSR hydration to avoid mismatch
